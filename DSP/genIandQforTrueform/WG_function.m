function WG_function(connectionID, data, fs, ArbFileName, chanNum)
...
ON_OFF_FILTER_CH1 = ['SOURce', num2str(chanNum) , ':FUNCtion:ARBitrary:FILTer ', 'OFF'];
...
fprintf(WG_obj, ['SOURce', num2str(chanNum) , ':DATA:VOLatile:CLEar']); 
...
header= ['SOURce', num2str(chanNum) , ':DATA:ARBitrary ', name, ', #', num2str(length(SENT_TO_WG_Bytes)), SENT_TO_WG_Bytes]; 
...
command = ['SOURce', num2str(chanNum) , ':FUNCtion:ARBitrary ' name];
% Выполнить команду
...
command = ['SOURCE', num2str(chanNum) , ':FUNCtion:ARB:SRATe ' num2str(sRate)];
% Выполнить команду
fprintf(WG_obj,command);

% Включить нашу функцию
fprintf(WG_obj,['SOURce', num2str(chanNum) , ':FUNCtion ARB']); 

% Установка амплитуды
command = ['SOURCE', num2str(chanNum) , ':VOLT ' num2str(amp)];
% Выполнить команду
fprintf(WG_obj,command);

% Установка смещения 
fprintf(WG_obj,['SOURCE', num2str(chanNum) , ':VOLT:OFFSET 0']);

% Включить выход 1 (если выход включен над ним загорается лампочка)
fprintf(WG_obj,['OUTPUT', num2str(chanNum) , ' ON']);

% Сообщить, что загрузка завершена
fprintf(['SENT_TO_WG waveform downloaded to channel ', num2str(chanNum) , '\n\n']);
